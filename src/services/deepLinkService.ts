...
async function consumeCallback(url: string): Promise<void> {
  ...
  if (callback.protocol !== 'com.motodrive.dz:' || callback.hostname !== 'auth' || callback.pathname !== '/callback') return;
...
export async function initializeDeepLinks(): Promise<void> {
  ...
  App.addListener('appUrlOpen', ({ url }) => {
    consumeCallback(url).catch((error) => console.error('OAuth deep-link error:', error));
  });
